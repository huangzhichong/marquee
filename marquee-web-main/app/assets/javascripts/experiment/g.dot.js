...
(function () {
    ...
    function _axis(x, y, length, from, to, steps, orientation, labels, type, dashsize, paper) {
        ...
        var path = type == "|" || type == " " ? ["M", x + .5, y, "l", 0, .001] : orientation == 1 || orientation == 3 ? ["M", x + .5, y, "l", 0, -length] : ["M", x, y + .5, "l", length, 0],
            ...
            i = ends.power,
            j = 0,
            txtattr = { font: "14px 'Grill Sans', Fontin-Sans, sans-serif" },
            text = paper.set(),
            d;
        ...
        if (+orientation == 1 || +orientation == 3) {
            var Y = y,
                addon = (orientation - 1 ? 1 : -1) * (dashsize + 4 + !!(orientation - 1));

            while (Y >= y - length) {
                type != "-" && type != " " && (path = path.concat(["M", x - (type == "+" || type == "|" ? dashsize : !(orientation - 1) * dashsize * 2), Y + .5, "l", dashsize * 2 + 1, 0]));
                text.push(paper.text(x + addon, Y, (labels && labels[j++])).attr(txtattr).attr({ "text-anchor": orientation - 1 ? "start" : "end" }));
                label += d;
                Y -= dx;
            }
            ...
            label = f;
            rnd = (i > 0) * i;
            addon = (orientation ? -1 : 1) * (dashsize + 45 + !orientation);
            ...
            while (X <= x + length) {
                ...
                text.push(txt = paper.text(X, y + addon, (labels && labels[j++]) || (' ')).rotate(270).attr(txtattr));
            ...
            if (Math.round(X - dx - x - length)) {
                ...
                text.push(paper.text(x + length, y + addon, (labels && labels[j]) || (Math.round(label) == label ? label : +label.toFixed(rnd))).rotate(270).attr(txtattr));
            }
        }
    ...
    Raphael.fn.dotchart = function (x, y, width, height, valuesx, valuesy, size, opts) {
        ...
        for (var i = 0, ii = valuesy.length; i < ii; i++) {
            var sym = this.raphael.is(symbol, "array") ? symbol[i] : symbol,
                ...
                Y = y + height - gutter - (valuesy[i] - miny) * ky - 30;
            ...
            console.log(X + " " + Y);
            this.text(X, Y + max + 40, opts.axisxlabels[i]).rotate(270).attr({font: "13px Grill Sans"});
        }
