...
#  jira_created                :datetime
#  jira_resolved               :datetime
#  priority                    :string(255)
#  severity                    :string(255)
#  issue_type                  :string(255)
#  environment_bug_was_found   :string(255)
#  who_found                   :string(255)
#  market                      :string(255)
#  planning_estimate_level_two :integer
#  components                  :string(255)
#  known_production_issue      :string(255)
#  created_at                  :datetime
#  updated_at                  :datetime
#
...
class JiraIssue < ActiveRecord::Base
  def self.platform
    where('jira_issues.key like ? or jira_issues.key like ? or jira_issues.key like ? or jira_issues.key like ? or jira_issues.key like ? or jira_issues.key like ?', 'FNDAMS%', 'FNDFIND%', 'USER%', 'EMAILZ%', 'COMMERCE%', 'AMOB%')
  end

  def self.ams
    where("jira_issues.key like 'FNDAMS%'")
  end

  def self.els
    where("jira_issues.key like 'FNDFIN%'")
  end

  def self.fusaus
    where("jira_issues.key like 'USER%'")
  end

  def self.email
    where("jira_issues.key like 'EMAILZ%'")
  end

  def self.commerce
    where("jira_issues.key like 'COMMERCE%'")
  end

  def self.mobile
    where("jira_issues.key like 'AMOB%'")
  end

  def self.endurance
    where('jira_issues.key like ? or jira_issues.key like ?' , 'FNDENDR%', 'FNDGVNG%')
  end

  def self.camps
    where('jira_issues.key like ?' , 'FNDCAMP%')
  end

  def self.sports
    where('jira_issues.key like ?' , 'FNDSPORTS%')
  end

  def self.swimming
    where("jira_issues.key like 'FNDSWIM%'")
  end
  ...
  def self.membership
    where("jira_issues.key like 'MEMB%'")
  end

  def self.framework
    where("jira_issues.key like 'FRAME%'")
  end

  def self.known_production_issue
    where("jira_issues.known_production_issue = 'Yes'")
  end
  ...
  def self.not_closed
    where('jira_issues.status != ?', 'Closed')
  end

  def self.resolved_today
    where('jira_issues.jira_resolved > ?', Date.today - 1)
  end

  def self.found_today
    where('jira_issues.jira_created > ?', Date.today - 1)
  end

  def self.resolved_90_days
    where('jira_issues.jira_resolved > ?', Date.today - 90)
  end

  def self.found_90_days
    where('jira_issues.jira_created > ?', Date.today - 90)
  end
  ...
  def self.production
    where('jira_issues.environment_bug_was_found like ?', 'Prod%')
  end

  def self.non_production
    where('jira_issues.environment_bug_was_found not like ?', 'Prod%')
  end

  def self.external
    where('jira_issues.who_found like ?', 'External%')
  end

  def self.internal
    where('jira_issues.who_found like ?', 'Internal%')
  end
