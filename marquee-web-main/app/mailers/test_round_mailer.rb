class TestRoundMailer < AsyncMailer
  ...
  def finish_mail(test_round_id)
    ...
    mail_to = []
    @project.mail_notify_settings.each do |mns|
      if mns.mail_notify_groups.include?(mail_notify_group) && mns.test_types.include?(@test_round.test_type)
        mail_to << mns
      end
    end
    ...
    if @test_round.result == 'pass'
      ...
      extra_mail_to = []
      @project.mail_notify_settings.each do |mns|
        if mns.mail_notify_groups.include?(extra_mail_notify_group) && mns.test_types.include?(@test_round.test_type)
          extra_mail_to << mns
        end
      end
    ...
    logger.warn "There's no mail notify settings for #{@test_round.project.name} on Test Type #{@test_round.test_suite.test_type.name}" if mail_to.empty?
