...
module FusionChartsHelper
  ...
  def render_chart(chart_swf,str_url,str_data,chart_id,chart_width,chart_height,debug_mode=false,register_with_js=false,options={})
    ...
    safe_concat("\t\t\t\t//Instantiate the Chart  \n\t\t\t\tif( FusionCharts.setCurrentRenderer ) FusionCharts.setCurrentRenderer(\""+renderer+"\");\n")

    safe_concat("\t\t\t\tvar chart_"+chart_id+"=new FusionCharts('"+chart_swf+"','"+chart_id+"','"+chart_width+"','"+chart_height+"',"+debug_mode_num+","+register_with_js_num+", '"+color+"', '"+scale_mode+"', '"+lang+"', '"+detect_flash_version+"', '"+auto_install_redirect+"');\n")

    if str_data.empty?
      safe_concat("\t\t\t\t<!-- Set the dataURL of the chart -->\n")
      if data_format==""
        data_format="xmlurl"
      end
      safe_concat("\t\t\t\tchart_"+chart_id+".setChartDataUrl(\""+str_url+"\",\""+data_format+"\");\n")
      #logger.info("The method used is setDataURL.The URL is " + str_url)
    else
      safe_concat("\t\t\t\t<!-- Provide entire XML data using DataXML method -->\n")
      safe_concat("\t\t\t\t")
      if data_format==""
        data_format="xml"
      end
      safe_concat('chart_'+chart_id+'.setChartData(\''+str_data+'\',\''+data_format+'\');')
      safe_concat("\n")
    ...
    if w_mode=='transparent' || w_mode=='opaque'
      ...
      safe_concat("\t\t\t\tchart_"+chart_id+".setTransparent("+transparency+");\n")
    end

    safe_concat("\t\t\t\t<!-- Finally render the chart. -->\n")
    safe_concat("\t\t\t\tchart_"+chart_id+".render('"+chart_id+"Div');\n")
    safe_concat("\t\t</script>\n")
  ...
  def render_chart_html(chart_swf,str_url,str_data,chart_id,chart_width,chart_height,debug_mode=false,register_with_js=false)
    ...
    safe_concat(content_tag("object","\n\t\t\t\t"+param_tag1+"\n\t\t\t\t"+param_tag2+"\n\t\t\t\t"+param_tag3+"\n\t\t\t\t"+param_tag4+"\n\t\t\t\t"+param_tag5+"\n\t\t\t\t"+param_tag6+"\n\t\t\t\t"+embed_tag+"\n\t\t",object_attributes))
