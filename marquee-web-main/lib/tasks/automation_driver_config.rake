...
task :add_initial_adc => :environment do
  ...
  adc_bvt = AutomationDriverConfig.create(:name => "Camps_BVT", :project_id => p.id, :automation_driver_id => driver1.id, :source_control => source_control, :source_paths => source_paths_bvt, :script_main_path => script_main_path_bvt)
  adc_regression = AutomationDriverConfig.create(:name => "Camps_Regression", :project_id => p.id, :automation_driver_id => driver1.id, :source_control => source_control, :source_paths => source_paths_regression, :script_main_path => script_main_path_regression)  
end