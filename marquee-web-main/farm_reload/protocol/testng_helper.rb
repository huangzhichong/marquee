...
class TestngHelper
  ...
  MARQUEE_LISTNER_JAR_PATH = TESTNG_CONFIG["marquee_listener_path"]
  ...
  def initialize(ruby_path, server_ip)
    @ruby_path = ruby_path
    @server_ip = server_ip
    # set_system_environment SCRIPT_CLASS_FOLDER_PATH+";"+TESTNG_JAR_PATH+";"+MARQUEE_LISTNER_JAR_PATH+";"+DOM_JAR_PATH+";"+HTTPCLIENT_JAR_PATH+";"+HTTPCORE_JAR_PATH+";"+SELENIUM__SERVER_JAR_PATH
  end
  ...
  def execute_testng(msg)
    ...
    if msg.match(/auto-run .*/)
      ...
      if script_file_path
        ...
        system "java -DRoundId=#{test_round_id} -DPlanName=#{automation_script_name} org.testng.TestNG -listener com.framework.marquee.LoggingListener #{test_xml}"
