...
set :application, "marquee-web-main"
set :repository,  "git@10.109.2.114:marquee_reload.git"
set :branch, "master"
...
set :deploy_to, "/home/active/marquee_deploy"
set :bundle_gemfile,  "marquee-web-main/Gemfile"
# set :bundle_dir, File.join(fetch(:shared_path), "cached-copy", "marquee-web-main")
set :bundle_dir, File.join(fetch(:current_path), "marquee-web-main")
...
set(:latest_release)  { fetch(:current_path, "marquee-web-main") }
set(:release_path)    { fetch(:current_path, "marquee-web-main") }
set(:current_release) { fetch(:current_path, "marquee-web-main") }
...
def run_rake(cmd)
  run "cd #{current_path}/marquee-web-main; bundle exec #{cmd}"
end
...
namespace :deploy do
  ...
  task :bundle do
    run "cd #{current_path}/marquee-web-main; bundle install"
  end
  ...
  task :restart do
    unless remote_file_exists?("#{current_path}/marquee-web-main/tmp")
      run "mkdir #{current_path}/marquee-web-main/tmp"
    end
    run "cd #{current_path}/marquee-web-main/tmp; rm restart.txt; touch restart.txt"
  end
  ...
  task :update_code, :except => { :no_release => true } do
    ...
    if fetch(:normalize_asset_timestamps, true)
      ...
      asset_paths = fetch(:public_children, %w(images stylesheets javascripts)).map { |p| "#{latest_release}/marquee-web-main/public/#{p}" }.join(" ")
